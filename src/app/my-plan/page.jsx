...
export default function MyPlan() {
  ...
  // Hydration ফিক্স (যাতে LocalStorage লোড হওয়ার আগে এরর না দেয়)
  useEffect(() => {
    setMounted(true);
  }, []);
  ...
  // Challenge C3: Delete/Done ফাংশনালিটি
  ...
  // Metrics ক্যালকুলেশন (শুধুমাত্র Today's Plan এর জন্য)[cite: 3, 4]
  ...
  // Active Tab অনুযায়ী ডেটা ফিল্টার ও সর্টিং[cite: 3, 4, 5]
  ...
  return (
    <div className="max-w-6xl mx-auto px-6 md:px-12 py-12 flex-grow w-full">
      {/* Header */}
      <div className="mb-8">
        ...
        <div className="space-y-4">
          {sortedData.map((workout) => (
            <div key={workout.id} className="bg-[#1e1e1e] border border-gray-800 rounded-2xl p-4 flex flex-col sm:flex-row items-center gap-6 hover:border-gray-600 transition-colors">
              ...
              {/* Thumbnail */}
              <div className="w-full sm:w-24 h-24 flex-shrink-0 bg-gray-800 rounded-xl overflow-hidden">
                <img src={workout.image || "https://via.placeholder.com/150"} alt={workout.name} className="w-full h-full object-cover" />
              </div>
